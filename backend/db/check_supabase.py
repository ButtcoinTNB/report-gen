...
try:
    ...
    try:
        ...
        if columns.data:
            ...
            # Check for report_id
            report_id_col = next((col for col in columns.data if col['column_name'] == 'report_id'), None)
            if report_id_col:
                print("\n✅ report_id column exists with type:", report_id_col['data_type'])
            else:
    ...
    try:
        ...
        """
        indexes = supabase.table("pg_indexes") \
            .select("indexname,indexdef") \
        ...
        if indexes.data:
            ...
            report_id_index = any('report_id' in idx.get('indexdef', '') for idx in indexes.data)
            if report_id_index:
    ...
    try:
        ...
        """
        triggers = supabase.table("information_schema.triggers") \
            .select("trigger_name,event_manipulation,action_statement") \
        ...
        if triggers.data:
            ...
            for trg in triggers.data:
                print(f"- {trg['trigger_name']} ({trg['event_manipulation']}): {trg['action_statement']}")
