        ...
        if result.data:
            ...
            try:
                ...
                if hasattr(columns, 'columns') and columns.columns:
                    ...
                    if required_columns:
                        missing_columns = [col for col in required_columns if col not in columns.columns]
                        if missing_columns:
                            print(f"❌ Missing required columns: {', '.join(missing_columns)}")
                        else:
                            print(f"✅ All required columns present")
...
def check_index(supabase, table_name, column_name):
    ...
    try:
        ...
        index_test = supabase.table(table_name).select("id").order(column_name).limit(1).execute()
    ...
    try:
        ...
        if test_insert.data and len(test_insert.data) > 0:
            ...
            if inserted_id:
                ...
                supabase.table(table_name).delete().eq("report_id", inserted_id).execute()
                return True
            else:
            ...
            try:
                ...
                if hasattr(cols, 'columns'):
                    ...
                    if table_name in required_cols:
                        missing = [col for col in required_cols[table_name].keys() if col not in columns]
                        if missing:
                            ...
                            for col in missing:
                                ...
                                if table_name == 'files' and col in ['file_id', 'report_id']:
                                    index_type = 'UNIQUE' if col == 'file_id' else ''
                                    sql += f"\nCREATE {index_type} INDEX idx_{table_name}_{col} ON {table_name}({col});"
                                elif table_name == 'reports' and col in ['report_id', 'template_id']:
                                ...
                                elif table_name == 'reference_reports' and col == 'reference_id':
                                    sql += f"\nCREATE UNIQUE INDEX idx_{table_name}_{col} ON {table_name}({col});"
                                print(sql)
                        else:
                        ...
                        try:
                            ...
                            for col, is_unique in index_columns.get(table_name, []):
                                if col in columns:
                                    try:
                                        index_test = supabase.table(table_name).select("id").order(col).limit(1).execute()
                                        print(f"  ✅ {col} is indexed" + (" (UNIQUE)" if is_unique else ""))
                                    except Exception as e:
                                        print(f"  ❌ {col} might not be indexed:", str(e))
                                else:
...
def generate_sql_commands(table_info, table_name):
    ...
    missing_columns = {
        col: spec for col, spec in required_columns[table_name].items()
    ...
    if 'file_id' in missing_columns:
        sql_commands.append(f"CREATE UNIQUE INDEX IF NOT EXISTS idx_{table_name}_file_id ON {table_name}(file_id);")
    if 'report_id' in missing_columns:
        sql_commands.append(f"CREATE INDEX IF NOT EXISTS idx_{table_name}_report_id ON {table_name}(report_id);")
...
def main():
    ...
    try:
        ...
        try:
            ...
            print("Existing buckets:", ", ".join(existing_buckets) if existing_buckets else "None")
