...
def check_env():
    ...
    if os.environ.get('RENDER') == 'true':
        print("⚠️ Running in Render environment! This script should be run before deployment.")
        return False
    return True
...
def main():
    ...
    if os.path.exists(verify_imports):
        ...
        print(f"Note: Import verification is informational only and won't block deployment")
    else:
    ...
    if success:
        ...
        print("   - Start command: python -m uvicorn main:app --host 0.0.0.0 --port $PORT")
        return 0
    else:
