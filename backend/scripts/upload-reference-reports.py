...
def store_in_supabase(report_name, extracted_text):
    ...
    try:
        ...
        print(
            f"  - Uploading to Supabase: {report_name} ({len(extracted_text)} characters)"
        )
...
def process_reports(folder_path):
    ...
    if success_count > 0:
        ...
        print(
            "These will now be used as formatting templates for generating insurance reports."
        )
    else:
...
if __name__ == "__main__":
    ...
    if not os.path.exists(default_folder):
        ...
        for path in alternative_paths:
            ...
            print(
                f"Warning: Reference reports folder {os.path.abspath(folder)} not found."
            )
    ...
    if not pdf_files:
        ...
        print(
            "Please add your reference report PDFs to the folder and run this script again."
        )
    else:
