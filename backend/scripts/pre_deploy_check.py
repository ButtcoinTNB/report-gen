...
IMPORT_PATTERN = re.compile(
    r"^(from|import)\s+([a-zA-Z0-9_.]+)\s*(import\s+|as\s+|$)", re.MULTILINE
)
        ...
        for file in files:
            if file.endswith(".py"):
                ...
                for match in IMPORT_PATTERN.finditer(content):
                    ...
                    if (
                        "." not in module_path
                        or module_path.split(".")[0] not in MODULES_TO_PREFIX
                    ):
                        continue
...
def check_init_files() -> List[Dict]:
    ...
    for root, dirs, _ in os.walk(BACKEND_DIR):
        ...
        if "__init__.py" not in os.listdir(root):
            ...
            issues.append(
                {"directory": relative_path, "issue": "Missing __init__.py file"}
            )
...
def main():
    ...
    if warnings > 0 or errors > 0:
        ...
        if relative_import_issues:
            print(
                "  - Run `python backend/scripts/fix_imports.py` to fix relative imports"
            )
        if init_file_issues:
            print(
                "  - Run `python backend/scripts/create_init_files.py` to create missing __init__.py files"
            )
