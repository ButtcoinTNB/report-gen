...
try:
    ...
    from services import ai_service, pdf_extractor, pdf_formatter
...
def check_environment():
    ...
    # Check directories
    dirs_to_check = [settings.UPLOAD_DIR, settings.GENERATED_REPORTS_DIR]
...
async def test_openrouter_api():
    ...
    try:
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": "Respond with 'API test successful' to confirm this test.",
            },
        ]
...
def test_pdf_extraction():
    ...
    test_pdf_path = os.path.join(
        settings.GENERATED_REPORTS_DIR, "test_pdf_extraction.pdf"
    )
    ...
    try:
        ...
        if (
            "PDF Extraction Test" in extracted_text
            and "This is a test PDF file" in extracted_text
        ):
            logger.info(f"✅ PDF extraction successful: {extracted_text[:50]}...")
            return True
        else:
            logger.error(
                f"❌ PDF extraction failed to extract expected text: {extracted_text}"
            )
            return False
...
def test_pdf_generation():
    ...
    test_output_path = os.path.join(
        settings.GENERATED_REPORTS_DIR, "test_generation.pdf"
    )
    ...
    try:
        ...
        # Test PDF generation
        output_path = pdf_formatter.generate_pdf(
            test_content, "test_generation.pdf", ref_metadata
        )

        if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            logger.info(
                f"✅ PDF generation successful: {output_path} ({os.path.getsize(output_path)} bytes)"
            )
            ...
            if pdf_libs.get("weasyprint") and pdf_libs.get("pdfrw"):
                if pdf_libs.get("weasyprint_pdfrw_compatible", False):
                    ...
                    logger.warning(
                        f"⚠️ WeasyPrint and pdfrw compatibility issue: {pdf_libs.get('compatibility_error', 'Unknown error')}"
                    )

            return True
        else:
            logger.error(
                f"❌ PDF generation failed: {output_path} does not exist or is empty"
            )
            return False
...
def test_error_handler():
    ...
    try:
        ...
        if (
            "ValueError" in str(e)
            and "Test error" in str(e)
            and "test operation" in str(e)
        ):
...
async def run_diagnostics():
    ...
    logger.info(
        f"PDF extraction check: {'✅ PASSED' if pdf_extraction_check else '❌ FAILED'}"
    )
    logger.info(
        f"PDF generation check: {'✅ PASSED' if pdf_generation_check else '❌ FAILED'}"
    )
    ...
    if total_passed == 6:
        ...
        logger.warning(
            "⚠️ Some diagnostic checks failed. See the logs above for details."
        )
        return False
...
if __name__ == "__main__":
    try:
        ...
        logger.critical(
            f"Diagnostic script encountered an unhandled exception: {e}", exc_info=True
        )
        sys.exit(1)
