...
class ImportVerifier:
    ...
    def scan_directory(self, directory):
        ...
        for file_path in files:
            ...
            if file_path.name == "__init__.py" or file_path.name in [
                "schemas.py",
                "openapi_examples.py",
            ]:
                continue
    ...
    def scan_file(self, file_path):
        ...
        try:
            ...
            missing_std_libs = [
                lib for lib in used_std_libs if not self.is_imported(lib, imports)
            ]
            ...
            for module, names in COMMON_IMPORTS.items():
                for name in names:
                    ...
                    ) and not self.is_name_imported(name, imports):
            ...
            if missing_std_libs:
                self.log(
                    f"❌ Missing standard library imports in {file_path}: {', '.join(missing_std_libs)}"
                )
                file_issues += 1
...
"""
            ...
            if re.search(r"from \w+|import \w+", content):
                ...
                content = re.sub(
                    r"(from \w+|import \w+)", hybrid_template + r"\1", content, count=1
                )
            else:
                # Add at the beginning of the file after docstring
                content = re.sub(
