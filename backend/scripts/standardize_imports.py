...
class ImportStandardizer:
    ...
    def process_directory(self, directory):
        ...
        python_files = [
            f for f in glob.glob(os.path.join(directory, "*.py"))
            if not os.path.basename(f) in ["__init__.py", "schemas.py", "openapi_examples.py"]
        ]
    ...
    def process_file(self, file_path):
        ...
        if re.search(r'try:.*?except\s+ImportError:', content, re.DOTALL):
            self.log(f"  ✓ Already has hybrid import pattern", 1)
            return
    ...
    def standardize_imports(self, content, file_path):
        ...
        for import_from, import_what in backend_imports:
            ...
            if import_from.startswith('backend.'):
                ...
                hybrid_backend_imports.append(f"from {import_from} import {import_what}")
        ...
        for module_parts in COMMON_HYBRID_MODULES:
            ...
            already_added = any(
                module == import_from.replace('backend.', '') and
                all(imp.strip() in import_what for imp in imports.split(','))
                for import_from, import_what in backend_imports
            )
        ...
        existing_imports = re.findall(import_pattern, content, re.MULTILINE)
        ...
        if backend_import_patterns:
            ...
            content = re.sub(rf'^({backend_pattern}).*?$', '', content, flags=re.MULTILINE)
        ...
        from_imports = re.findall(r'^(from (?!backend).+? import .+?$)', content, re.MULTILINE)
