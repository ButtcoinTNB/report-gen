    ...
    for pdf_path in pdf_files:
        try:
            ...
            if not text_content or len(text_content) < 100:
                print(f"Warning: {file_name} has very little text content ({len(text_content)} chars). Check the file.")
            else:
                print(f"Extracted {len(text_content)} characters of text from {file_name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dir', type=str, default='backend/reference_reports',
    ...
    if result:
        print("PDFs uploaded successfully! The AI will now use these as reference templates.")
    else:
