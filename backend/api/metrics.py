...
async def get_metrics_summary():
    ...
    try:
        ...
        resource_count = sum(
            len(resources) for resources in resource_manager.resources.values()
        )
        ...
        return {
            ...
            "document_generation": {
                ...
                "cache_hit_ratio": (
                    ...
                    / max(
                        ...
                        (
                            docx_metrics.get("cache_hits", 0)
                            + docx_metrics.get("cache_misses", 0)
                        ),
                    )
                ),
            },
            "report_generation": {
                ...
                "error_rate": len(error_metrics)
                / max(1, total_reports + len(error_metrics)),
            },
            "resources": {
            ...
            "system": {
                "uptime": (
                    time.time()
                    - metrics_collector.get_metrics("system_startup", limit=1)[0].get(
                        "timestamp", time.time()
                    )
                    if metrics_collector.get_metrics("system_startup", limit=1)
                    else 0
                )
            },
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Error generating metrics summary: {str(e)}"
        )
...
async def get_document_metrics(limit: int = Query(10, ge=1, le=100)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error generating document metrics: {str(e)}"
        )
...
async def get_report_metrics(limit: int = Query(10, ge=1, le=100)):
    ...
    try:
        ...
        error_metrics = metrics_collector.get_metrics(
            "report_generation_error", limit=limit
        )
        ...
        if report_metrics:
            avg_generation_time = sum(
                m.get("duration", 0) for m in report_metrics
            ) / len(report_metrics)
            avg_quality_score = sum(
                m.get("quality_score", 0) for m in report_metrics
            ) / len(report_metrics)
            avg_iterations = sum(m.get("iterations", 0) for m in report_metrics) / len(
                report_metrics
            )
        else:
            avg_generation_time = 0
        ...
        raise HTTPException(
            status_code=500, detail=f"Error generating report metrics: {str(e)}"
        )
...
async def record_startup():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error recording startup: {str(e)}"
        )
