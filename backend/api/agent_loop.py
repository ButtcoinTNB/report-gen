...
try:
    ...
    from backend.utils.error_handler import ErrorHandler, raise_error
...
async def process_report_generation(
    ...
    try:
        ...
        await update_task_status(
            task_id, "processing", "Initializing AI agents", progress=10
        )
        ...
        try:
            ...
            logger.info(
                f"Agent loop completed in {processing_time:.2f} seconds after {result['iterations']} iterations"
            )
            ...
            docx_result = await generate_docx_async(result["draft"], docx_path)
            ...
            await update_task_status(
                task_id, "failed", f"Error generating report: {str(e)}", error=str(e)
            )
            raise
...
async def process_report_refinement(
    ...
    try:
        # Update task status to processing
        await update_task_status(
            task_id, "processing", "Starting report refinement", 0.05
        )
    ...
    try:
        ...
        agent_loop = AIAgentLoop(
            writer_system_prompt=(
                request.writer_system_prompt
                if hasattr(request, "writer_system_prompt")
                else None
            ),
            reviewer_system_prompt=(
                request.reviewer_system_prompt
                if hasattr(request, "reviewer_system_prompt")
                else None
            ),
        )
