...
async def download_report(
    report_id: UUID4,
    format: str = Query(
        "docx", description="Format of the report, either 'docx' or 'pdf'"
    ),
...
async def get_report_file_info(report_id: UUID4):
    ...
    async with async_supabase_client_context() as supabase:
        ...
        content_type = (
            FileProcessor.get_mime_type(content_file_path)
            if file_exists
            else "text/plain"
        )
...
async def download_report_content(report_id: UUID4):
    ...
    async with async_supabase_client_context() as supabase:
        ...
        if os.path.exists(content_file_path):
            ...
            logger.info("Report file not found on disk, using content from database")
...
async def download_file(
    bucket: str, file_path: str, current_user: dict = Depends(get_current_user)
):
