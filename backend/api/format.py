...
def update_report_file_path(report_id: UUID4, file_path: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error updating report file path: {str(e)}"
        )
    ...
    try:
        ...
        return APIResponse(
            ...
            data=FormatPreviewResponse(
                html=pdf_result["html"], metadata=reference_metadata
            ),
...
def get_reference_metadata():
    ...
    """
    reference_metadata = {"headers": [], "footers": []}

    # List of directories to check for reference PDFs
    search_paths = ["reference_reports", safe_path_join("backend", "reference_reports")]
                ...
                try:
                    with open(metadata_path, "r") as f:
                        ...
                        if "report_content" in metadata:
                            ...
                            print(
                                f"Using report content from metadata.json for report_id {report_id}"
                            )
                except Exception as e:
                ...
                if is_uuid:
                    ...
                    if os.path.exists(content_path):
                        try:
                            ...
                            print(
                                f"Using content from content.txt for report_id {report_id}"
                            )
...
This error occurs when the system cannot locate the report content in the database or in local storage."""
        ...
        result = await format_report_as_docx(
            report_content, reference_metadata, filename=docx_filename
        )
        ...
        if is_uuid:
            try:
                ...
                metadata["report_content"] = (
                    report_content  # Save the content directly in metadata
                )
        ...
        try:
            ...
            if not is_uuid:
                raise e
            print(
                f"Warning: Could not update database for report_id {report_id}: {str(e)}"
            )
        ...
        if not os.path.exists(absolute_docx_path):
            raise HTTPException(
                status_code=404,
                detail=f"Generated DOCX not found at {absolute_docx_path}",
            )
...
async def preview_file(data: Dict = Body(...)):
    ...
    pdf_path = await format_report_as_pdf(
        report_content, reference_metadata, is_preview=True, filename=output_filename
    )
    ...
    return {"preview_id": preview_id, "preview_url": preview_url, "pdf_path": pdf_path}
...
@router.get(
    "/preview-file/{preview_id}", response_model=APIResponse[PreviewFileResponse]
)
@api_error_handler
...
@router.get(
    "/preview-file/{preview_id}/info", response_model=APIResponse[PreviewFileResponse]
)
@api_error_handler
async def get_preview_file_info(preview_id: str):
    ...
    return {"file_path": file_path, "preview_id": preview_id}
