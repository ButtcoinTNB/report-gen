...
class DocxService:
    def __init__(self):
        ...
        for template_dir in self.templates_dirs:
            try:
                ...
                logger.info(
                    f"Note: Could not create template directory {template_dir}: {str(e)}"
                )
                # Check if directory exists anyway (might be read-only)
                if template_dir.exists():
                    logger.info(
                        f"Directory {template_dir} exists but cannot be created (might be read-only)"
                    )
    ...
    def find_template(self, template_name: str = "template.docx") -> Optional[Path]:
        ...
        for template_dir in self.templates_dirs:
            ...
            try:
                ...
                logger.warning(
                    f"Skipping potentially unsafe template path: {template_dir}/{template_name}"
                )
                    ...
                    for file_name in os.listdir(template_dir):
                        if file_name.lower().endswith(".docx"):
                            try:
                                ...
                                logger.warning(
                                    f"Skipping potentially unsafe template file: {file_name}"
                                )
                except Exception as e:
    ...
    def generate_report(
        self, template_variables: Dict[str, Any], template_name: str = "template.docx"
    ) -> str:
        """
        ...
        try:
            ...
            if not template_path:
                logger.warning(
                    f"Template {template_name} not found, creating a basic document instead"
                )
                ...
                try:
                    temp_template = safe_path_join(
                        self.output_dir, "temp_template.docx"
                    )
    ...
    def modify_report(
        self, report_id: UUID4, template_variables: Dict[str, Any]
    ) -> str:
        """
