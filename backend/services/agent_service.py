...
class AgentService:
    ...
    async def generate_report(self, report_data: ReportCreate) -> Dict[str, Any]:
        ...
        await task_manager.schedule_task(
            ...
            metadata={"report_id": report_id, "file_count": len(report_data.file_ids)},
        )

        return {"report_id": report_id, "task_id": task_id, "status": "processing"}

    async def _run_agent_loop(
        self, report_id: str, report_data: ReportCreate
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            while (
                current_iteration < max_iterations and quality_score < quality_threshold
            ):
                current_iteration += 1
                ...
                draft_content = await self._run_writer_agent(
                    document_content, current_iteration
                )

                # Reviewer agent - evaluate quality (CPU/memory intensive)
                review_result = await self._run_reviewer_agent(
                    document_content, draft_content
                )
                ...
                document_content = self._incorporate_feedback(
                    document_content, feedback
                )
    ...
    async def _run_writer_agent(
        self, document_content: Dict[str, Any], iteration: int
    ) -> str:
        """
    ...
    async def _run_reviewer_agent(
        self, document_content: Dict[str, Any], draft_content: str
    ) -> Dict[str, Any]:
        """
    ...
    def _incorporate_feedback(
        self, document_content: Dict[str, Any], feedback: str
    ) -> Dict[str, Any]:
        """
    ...
    async def cancel_report_generation(self, report_id: str) -> bool:
        ...
        self._update_agent_state(
            report_id,
            {"status": "cancelled", "last_update": datetime.utcnow().isoformat()},
        )
