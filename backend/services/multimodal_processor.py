    ...
    try:
        ...
        for page_num in range(max_pages):
            try:
                ...
                # Save the image
                image_path = safe_path_join(
                    output_dir, f"pdf_page_{page_num + 1:03d}.png"
                )
                pix.save(image_path)
                ...
                logger.error(
                    f"Error converting PDF page {page_num + 1}: {str(page_error)}"
                )
                continue
...
def convert_docx_to_images(docx_path: str, output_dir: str) -> List[str]:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Attempting to convert DOCX to PDF using LibreOffice: {docx_path}"
            )
            ...
            if result.returncode == 0:
                ...
                if process.returncode == 0:
                    ...
                    if os.path.exists(converted_pdf):
                        ...
                        logger.info(
                            f"Successfully converted DOCX to PDF using LibreOffice: {temp_pdf_path}"
                        )
                    else:
                        logger.warning(
                            f"LibreOffice conversion completed but PDF not found at {converted_pdf}"
                        )
                else:
                    logger.warning(f"LibreOffice conversion failed: {process.stderr}")
            else:
                logger.warning(
                    "LibreOffice not found, trying alternative conversion methods"
                )
        ...
        if not convert_success:
            try:
                ...
                logger.info(
                    f"Attempting to convert DOCX to PDF using docx2pdf: {docx_path}"
                )
                ...
                if os.path.exists(temp_pdf_path) and os.path.getsize(temp_pdf_path) > 0:
                    convert_success = True
                    logger.info(
                        f"Successfully converted DOCX to PDF using docx2pdf: {temp_pdf_path}"
                    )
                else:
        ...
        if not image_paths:
            logger.warning(
                "PDF conversion methods failed. Falling back to direct DOCX rendering"
            )
            ...
            try:
                ...
                for i, part in enumerate(html_parts):
                    ...
                    except Exception:  # Catch font loading errors
                        # Fallback to default
                    ...
                    # Save the image
                    image_path = safe_path_join(
                        output_dir, f"docx_page_{i + 1:03d}.png"
                    )
                    img.save(image_path)
                    image_paths.append(image_path)
                    logger.info(
                        f"Created image for DOCX content part {i + 1}: {image_path}"
                    )
        ...
        if os.path.exists(temp_pdf_path):
            ...
            except Exception:  # Catch file deletion errors
                pass
...
async def process_document_with_multimodal_api(
    file_path: str, prompt: str, system_message: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="doc_images_") as temp_dir:
            ...
            logger.info(
                f"Generated {len(image_paths)} images in {time.time() - start_time:.2f} seconds"
            )
            ...
            for i in range(max_images):
                content_items.append(
                    {"type": "image_url", "image_url": {"url": base64_images[i]}}
                )
...
async def extract_text_with_multimodal(file_path: str) -> str:
    ...
    response = await process_document_with_multimodal_api(
        file_path, prompt, system_message
    )
    ...
    if (
        ...
        logger.info(
            f"Successfully extracted {len(extracted_text)} characters with multimodal API"
        )
