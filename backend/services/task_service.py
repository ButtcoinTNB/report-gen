...
class TaskService:
    ...
    async def create_task(
        self, db: AsyncSession, task_type: str, params: Dict[str, Any] = None
    ) -> Task:
    ...
    async def update_task(
        self, db: AsyncSession, task_id: UUID4, updates: Dict[str, Any]
    ) -> Optional[Task]:
    ...
    async def count_tasks(self, db: AsyncSession, status: Optional[str] = None) -> int:
    ...
    async def delete_expired_tasks(
        ...
        try:
            ...
            result = await db.execute(delete(Task).where(Task.expires_at < cutoff_date))
            await db.commit()
    ...
    async def _handle_document_processing(
        ...
        await self._update_progress(
            db, task.id, 10, "Extracting text from documents", ProcessStage.EXTRACTION
        )
        await asyncio.sleep(2)  # Simulate work

        await self._update_progress(
            db, task.id, 40, "Analyzing document content", ProcessStage.ANALYSIS
        )
        await asyncio.sleep(3)  # Simulate work

        await self._update_progress(
            db, task.id, 70, "Organizing data", ProcessStage.ANALYSIS
        )
        await asyncio.sleep(2)  # Simulate work
    ...
    async def _handle_report_generation(
        ...
        await self._update_progress(
            db, task.id, 10, "Analyzing documents", ProcessStage.ANALYSIS
        )
        await asyncio.sleep(2)  # Simulate work

        await self._update_progress(
            db, task.id, 30, "Generating initial report draft", ProcessStage.WRITER
        )
        await asyncio.sleep(3)  # Simulate work

        await self._update_progress(
            db, task.id, 60, "Reviewing report content", ProcessStage.REVIEWER
        )
        await asyncio.sleep(2)  # Simulate work

        await self._update_progress(
            db, task.id, 80, "Finalizing report", ProcessStage.FORMATTING
        )
        await asyncio.sleep(2)  # Simulate work
    ...
    async def _handle_report_refinement(
        ...
        await self._update_progress(
            db, task.id, 20, "Analyzing refinement instructions", ProcessStage.ANALYSIS
        )
        await asyncio.sleep(2)  # Simulate work

        await self._update_progress(
            db, task.id, 50, "Applying refinements to report", ProcessStage.REFINEMENT
        )
        await asyncio.sleep(3)  # Simulate work

        await self._update_progress(
            db, task.id, 80, "Formatting updated report", ProcessStage.FORMATTING
        )
        await asyncio.sleep(2)  # Simulate work
    ...
    async def _handle_report_export(
        ...
        await self._update_progress(
            ...
            task.id,
            30,
            f"Preparing report for {format} export",
            ProcessStage.FORMATTING,
        )
        await asyncio.sleep(2)  # Simulate work

        await self._update_progress(
            db, task.id, 70, "Generating final document", ProcessStage.FINALIZATION
        )
        await asyncio.sleep(2)  # Simulate work
