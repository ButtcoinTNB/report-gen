...
class DocxFormatter:
    ...
    def _load_templates(self) -> Dict[str, Path]:
        ...
        for template_file in TEMPLATE_DIR.glob("*.docx"):
            template_type = template_file.stem  # Use filename without extension as template type
    ...
    async def _clean_cache(self) -> None:
        ...
        files = [(f, f.stat().st_atime) for f in CACHE_DIR.glob("*.docx") if f.is_file()]
        ...
        for file_path, _ in files:
            ...
            if total_size_mb < MAX_CACHE_SIZE_MB * 0.8:  # Stop if we're under 80% of max
                break
    ...
    def _add_custom_property(self, doc: Document, name: str, value: str) -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to add custom property {name}, using python-docx that doesn't support it")
    
    def _validate_document_quality(self, content: str, document: Document) -> Tuple[bool, Dict[str, Any]]:
        ...
        for paragraph in document.paragraphs:
            if paragraph.style.name.startswith('Heading'):
                ...
                if section_count > 1:  # Check if previous section was empty
                    if empty_sections > 0:
                        quality_metrics["issues"].append(f"Empty or minimal content in section {section_count-1}")
                empty_sections = 0
        ...
        if grammar_issues:
            quality_metrics["issues"].extend(grammar_issues[:3])  # Limit to top 3 issues
    ...
    async def generate_document(self, 
        ...
        if output_path is None:
            safe_title = re.sub(r'[^\w\-_\.]', '_', metadata.get("title", "report"))[:50]
            report_id = metadata.get("report_id", hashlib.md5(content[:100].encode()).hexdigest()[:8])
        ...
        if cached_doc:
            ...
            return {
                ...
                "quality": {"score": 1.0, "passed": True}  # Assume cached documents passed quality check
            }
        ...
        try:
            ...
            passed_quality, quality_metrics = self._validate_document_quality(content, document)
            ...
            self.metrics["avg_generation_time"] = (avg_time * (doc_count - 1) + generation_time) / doc_count
            ...
            logger.info(f"Document generated in {generation_time:.2f}s with quality score {quality_metrics['score']:.2f}")
            return result
    ...
    def _format_document(self, document: Document, content: str) -> None:
        ...
        if document.paragraphs and "heading" in document.paragraphs[0].style.name.lower():
...
async def generate_docx(content: str, output_path: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
