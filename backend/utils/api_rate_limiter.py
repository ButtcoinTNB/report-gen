...
class TokenBucket:
    ...
    def __init__(
        ...
        logger.info(f"Initialized rate limiter '{name}': {rate} requests/sec, capacity {capacity}")
    ...
    async def consume_or_wait(self, tokens: float = 1.0, max_wait: float = 10.0) -> bool:
        """
...
class ApiRateLimiter:
    ...
    def add_limiter(self, name: str, rate: float, capacity: float):
        ...
        with self._lock:
            ...
            logger.debug(f"Added rate limiter '{name}': {rate} req/s, capacity {capacity}")
    ...
    def get_limiter(self, name: str) -> TokenBucket:
        ...
        with self._lock:
            if name not in self.limiters:
                logger.warning(f"Rate limiter '{name}' not found, creating with default settings")
    ...
    async def wait_for_token(self, name: str, tokens: float = 1.0, max_wait: float = 10.0) -> bool:
        """
    ...
    def get_metrics(self) -> Dict[str, Dict[str, Any]]:
        ...
        with self._lock:
            return {name: limiter.metrics.copy() for name, limiter in self.limiters.items()}
    ...
    def cleanup_stale_limiters(self, max_age_seconds: int = 3600) -> List[str]:
        ...
        if stale_limiters:
            logger.info(f"Removed {len(stale_limiters)} stale rate limiters: {', '.join(stale_limiters)}")
...
def rate_limited(limiter_name: str, tokens: float = 1.0, max_wait: float = 10.0):
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            if not allowed:
                logger.warning(f"Rate limit exceeded for {limiter_name}, request dropped")
