...
class FileProcessor:
    ...
    @staticmethod
    def save_upload(
        file: BinaryIO, directory: Union[str, Path], filename: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def copy_file(source_path: Union[str, Path], target_path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error copying file from {source_str} to {target_str}: {str(e)}"
            )
            return False
    ...
    def init_chunked_upload(
        ...
        logger.info(
            f"Initialized chunked upload {upload_id} for {filename} with {total_chunks} chunks"
        )
        return upload_info

    @staticmethod
    def save_chunk(
        upload_id: str, chunk_index: int, chunk_data: BinaryIO
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Saved chunk {chunk_index} for upload {upload_id} ({upload_info['received_chunks']}/{upload_info['total_chunks']})"
        )
        return upload_info
    ...
    def complete_chunked_upload(
        ...
        try:
            ...
            logger.error(
                f"Error cleaning up chunk files for upload {upload_id}: {str(e)}"
            )
            # Continue despite cleanup failure

        logger.info(
            f"Completed chunked upload {upload_id}. Final file saved to {target_path}"
        )
        return {
            "upload_id": upload_id,
                ...
                if dir_name.startswith(f"chunks_{upload_id}"):
                    ...
                    if os.path.exists(metadata_path):
                        try:
                            ...
                            logger.error(
                                f"Error loading metadata for upload {upload_id}: {str(e)}"
                            )
                            return None
            ...
            for dir_name in dirs:
                if dir_name.startswith(f"chunks_{upload_id}"):
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error cleaning up chunked upload {upload_id}: {str(e)}"
                        )
                        return False
