...
class AIAgentLoop:
    def __init__(
        self, max_loops: int = 3, progress_callback: Optional[Callable] = None
    ):
    ...
    def log_event(self, event_type: str, details: Dict) -> None:
        ...
        event = {"timestamp": current_time, "type": event_type, **details}
    ...
    def cancel_processing(self) -> None:
        ...
        self.log_event(
            "cancel_requested", {"message": "Processing cancellation requested"}
        )
    ...
    def _load_template(self) -> str:
        ...
        template_path = (
            Path(__file__).parent.parent / "reference_reports" / "template.docx"
        )
        with open(template_path, "rb") as f:
            return f.read()

    async def _call_model(
        self, prompt: str, system_prompt: str, retries: int = None
    ) -> str:
        ...
        allowed = await rate_limiter.wait_for_token(
            "openrouter", tokens=1.0, max_wait=10.0
        )
        if not allowed:
            self.log_event(
                "rate_limit", {"message": "Rate limit exceeded, request dropped"}
            )
            raise Exception(
        ...
        for attempt in range(retries):
            ...
            try:
                self.log_event(
                    "api_call", {"attempt": attempt + 1, "model": DEFAULT_MODEL}
                )
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code == 429:
                        ...
                        rate_limiter.get_limiter("openrouter").metrics[
                            "throttled_requests"
                        ] += 1
                        await asyncio.sleep(
                            self.network_backoff_factor**attempt
                        )  # Exponential backoff
                        continue
                    ...
                    if response.status_code != 200:
                        ...
                        if response.status_code in [
                            ...
                            503,
                            504,
                        ]:  # Gateway errors, likely transient
                            if attempt < retries - 1:
                                delay = self.network_backoff_factor**attempt
                                logger.info(
                                    f"Transient error, retrying in {delay:.2f} seconds..."
                                )
                    ...
                    self.log_event(
                        "api_success",
                        {
                            ...
                            "tokens": response_data.get("usage", {}).get(
                                "total_tokens", 0
                            ),
                        },
                    )
                ...
                self.log_event(
                    "network_error",
                    {"attempt": attempt + 1, "error": str(e), "type": error_type},
                )
                logger.error(
                    f"Network error ({error_type}) on attempt {attempt + 1}: {e}"
                )
                ...
                if attempt < retries - 1:
                    ...
                    delay = self.network_backoff_factor**attempt
                    logger.info(
                        f"Retrying after network error in {delay:.2f} seconds..."
                    )
                    await asyncio.sleep(delay)
                    continue
                ...
                last_error = e
                self.log_event(
                    "api_exception", {"attempt": attempt + 1, "error": str(e)}
                )
    ...
    async def generate_report(self, user_content: str) -> Dict:
        ...
        try:
            ...
            self.log_event(
                "process_start",
                {"max_iterations": self.max_loops, "content_length": len(user_content)},
            )
            self._update_progress(
                5, "Inizializzazione del processo di generazione", "initializing"
            )
            ...
            for i in range(self.max_loops):
                if self.is_cancelling:
                    self.log_event(
                        "process_cancelled",
                        {"iteration": i, "message": "Process cancelled by user"},
                    )
                ...
                self.log_event(
                    "iteration_start", {"iteration": i + 1, "total": self.max_loops}
                )
                ...
                review_result = await self._call_model(
                    reviewer_input, self.reviewer_prompt
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Feedback score: {feedback['score']}, suggestions: {len(feedback['suggestions'])}"
                    )
                except json.JSONDecodeError:
                    self.log_event("reviewer_parse_error", {"iteration": i + 1})
                    logger.warning(
                        "Failed to parse reviewer feedback JSON, retrying with explicit JSON instruction"
                    )
                    ...
                    try:
                        feedback = json.loads(review_result)
                        self.log_event(
                            "reviewer_retry_success", {"score": feedback["score"]}
                        )
                    except json.JSONDecodeError:
                        self.log_event("reviewer_retry_failed", {})
                        feedback = {
                            "score": 0,
                            "suggestions": ["Errore nel formato del feedback"],
                        }
                        logger.error(
                            "Failed to parse reviewer feedback JSON even after retry"
                        )
                ...
                if len(iteration_times) > 0:
                    ...
                    estimated_remaining_seconds = (
                        avg_iteration_time * remaining_iterations
                    )
            ...
            return {"draft": draft, "feedback": feedback, "iterations": i + 1}
        except Exception as e:
            self.log_event(
                "process_error", {"error": str(e), "error_type": type(e).__name__}
            )
    ...
    async def refine_report(
        self, refinement_prompt: str, progress_callback=None
    ) -> Dict:
        """
        ...
        if content_start > -1 and instructions_start > -1:
            content = refinement_prompt[
                content_start
                + len("CONTENUTO ATTUALE DEL REPORT:") : instructions_start
            ].strip()
            instructions = refinement_prompt[
                instructions_start + len("ISTRUZIONI PER IL MIGLIORAMENTO:") :
            ].strip()
        else:
        ...
        if len(instructions.split()) < 20 and cache_key in self.refinement_cache:
            ...
            logger.info(
                f"Using cached refinement pattern (hash: {instruction_hash[:8]})"
            )
        ...
        compressed_brand_guide = "\n".join(
            [
                line
                for line in self.brand_guide.split("\n")
        ...
        for i in range(max_refinement_loops):
            ...
            if progress_callback:
                ...
                progress_callback(
                    iteration_progress,
                    f"Refinement iteration {i + 1}/{max_refinement_loops}",
                )
            ...
            if progress_callback:
                progress_callback(
                    iteration_progress + 0.15, "Generating refined content"
                )
            ...
            if progress_callback:
                progress_callback(
                    iteration_progress + 0.25, "Evaluating refinement quality"
                )
        ...
        if feedback["score"] > 0.8 and len(instructions.split()) < 20:
            ...
            if len(self.refinement_cache) > self.cache_max_size:
                ...
                oldest = sorted(
                    self.refinement_cache.items(), key=lambda x: x[1]["last_used"]
                )
                for key, _ in oldest[
                    : len(self.refinement_cache) - self.cache_max_size
                ]:
                    del self.refinement_cache[key]

            logger.info(
                f"Added refinement pattern to cache (hash: {instruction_hash[:8]})"
            )
