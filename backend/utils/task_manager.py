...
class TaskCache:
    ...
    def __init__(self, ttl_minutes: int = 60):
        ...
        self._resource_tracker = get_dependency_manager().register_tracker(
            "task_cache", lambda _: None, "task"  # No-op cleanup function
        )
    ...
    def _cleanup_task_resources(self, task_id: str):
        ...
        for file_key in ["input_file", "output_file", "temp_files"]:
            if file_key in task and task[file_key]:
                files = (
                    task[file_key]
                    if isinstance(task[file_key], list)
                    else [task[file_key]]
                )
                for file in files:
                    try:
        ...
        if "resource_paths" in task and task["resource_paths"]:
            for path in task["resource_paths"]:
                try:
                    ...
                    logger.error(
                        f"Error cleaning up resource {path} for task {task_id}: {e}"
                    )
        ...
        if "asyncio_tasks" in task and task["asyncio_tasks"]:
            for async_task in task["asyncio_tasks"]:
                try:
                    ...
                    logger.error(
                        f"Error cancelling asyncio task for task {task_id}: {e}"
                    )
        ...
        while not self._shutdown_requested:
            try:
                ...
                for task_id, task in self._cache.items():
                    ...
                    last_accessed = task.get(
                        "_last_accessed", task.get("_created_at", now)
                    )
    ...
    async def _persist_tasks(self):
        ...
        try:
            ...
            for task_id, task in self._cache.items():
                ...
                task_copy["_created_at"] = task.get(
                    "_created_at", datetime.now()
                ).isoformat()
                task_copy["_last_accessed"] = task.get(
                    "_last_accessed", datetime.now()
                ).isoformat()
