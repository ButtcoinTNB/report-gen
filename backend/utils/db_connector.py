...
class DBConnector:
    ...
    def __init__(
        ...
        get_dependency_manager().register_connection("db_connection", self, "close")
        ...
        try:
            with self._get_connection() as conn:
                ...
                if cursor.rowcount > 0:
                    logger.info(
                        f"Cleaned up {cursor.rowcount} expired tasks from database"
                    )
    ...
    async def set_in_cache(
        self, key: str, value: Any, ttl_minutes: Optional[int] = None
    ) -> None:
        """
