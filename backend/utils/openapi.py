...
def custom_openapi(
    app: FastAPI, endpoint_examples: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    ...
    openapi_schema["servers"] = [
        {
            ...
            "variables": {
                ...
                "host": {"default": settings.DOMAIN or "localhost:8000"},
            },
        },
        {
            "url": (
                ...
                for method, examples in methods.items():
                    if method.lower() in openapi_schema["paths"][path]:
                        add_examples_to_endpoint(
                            openapi_schema, path, method.lower(), examples
                        )
...
def add_examples_to_endpoint(
    openapi_schema: Dict[str, Any], path: str, method: str, examples: Dict[str, Any]
) -> None:
    """
    ...
    if (
        path not in openapi_schema["paths"]
        or method not in openapi_schema["paths"][path]
    ):
        return

    # Add request body examples
    if (
        "requestBody" in examples
        and "requestBody" in openapi_schema["paths"][path][method]
    ):
        if "content" in openapi_schema["paths"][path][method]["requestBody"]:
            for content_type in openapi_schema["paths"][path][method]["requestBody"][
                "content"
            ]:
    ...
    if "responses" in examples:
        for status_code, response_examples in examples["responses"].items():
            if status_code in openapi_schema["paths"][path][method]["responses"]:
                for content_type in openapi_schema["paths"][path][method]["responses"][
                    status_code
                ].get("content", {}):
                    if "application/json" in content_type:
                        # Add examples to responses
                        openapi_schema["paths"][path][method]["responses"][status_code][
                            "content"
                        ][content_type]["examples"] = response_examples
