...
/**
 * Proxy route for report preview formatting requests
...
export default async function handler(
  ...
  try {
    ...
    const backendUrl = `${config.API_URL}/api/format/preview-file`;
    console.log(`Forwarding format preview request to: ${backendUrl}`);
    ...
    console.error('Error forwarding format preview request:', error instanceof Error ? error.message : String(error));
