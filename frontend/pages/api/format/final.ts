...
/**
 * Proxy route for final report formatting requests
...
export default async function handler(
  ...
  try {
    ...
    const backendUrl = `${config.API_URL}/api/format/final`;
    console.log(`Forwarding final format request to: ${backendUrl}`);
    ...
    console.error('Error forwarding final format request:', error instanceof Error ? error.message : String(error));
