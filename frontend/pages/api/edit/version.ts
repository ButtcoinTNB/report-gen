...
/**
 * Proxy route for fetching a specific version of a report
...
export default async function handler(
  ...
  try {
    ...
    const backendUrl = `${config.API_URL}/api/edit/${reportId}/versions/${versionNumber}`;
    console.log(`Forwarding version request to: ${backendUrl}`);
    ...
    console.error('Error forwarding version request:', error instanceof Error ? error.message : String(error));
