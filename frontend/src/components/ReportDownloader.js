...
export default function ReportDownloader() {
  ...
  const handleGeneratePreview = async () => {
    ...
    try {
      ...
      dispatch(setError(err.message || 'Failed to generate preview'));
      dispatch(setLoading({
        isLoading: false,
        stage: 'error'
      }));
  ...
  // Format final report
  const handleFormatReport = async (format = 'docx') => {
    if (!reportId) {
    ...
    try {
      ...
      dispatch(setError(err.message || 'Failed to format report'));
      dispatch(setLoading({
        isLoading: false,
        stage: 'error'
      }));
  ...
  const handleDownload = async (format = 'docx') => {
    ...
    try {
      ...
      dispatch(setError(err.message || 'Failed to download report'));
      dispatch(setLoading({
        isLoading: false,
        stage: 'error'
      }));
