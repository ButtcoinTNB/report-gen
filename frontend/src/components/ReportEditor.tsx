...
const ReportEditor: React.FC = () => {
  ...
  const handleSaveEdits = async (): Promise<void> => {
    ...
    try {
      ...
      dispatch(setError(err instanceof Error ? err.message : 'Failed to save edits'));
      dispatch(setLoading({
        isLoading: false,
        stage: 'error'
      }));
  ...
  // AI refinement
  const handleAIRefine = async (): Promise<void> => {
    if (!reportId) {
    ...
    try {
      ...
      dispatch(setError(err instanceof Error ? err.message : 'Failed to refine report'));
      dispatch(setLoading({
        isLoading: false,
        stage: 'error'
      }));
